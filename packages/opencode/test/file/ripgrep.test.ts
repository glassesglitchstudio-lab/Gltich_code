...
describe("file.ripgrep", () => {
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, ".glitchcode", "thing.json"), "{}")
      },
    })
    ...
    expect(files.includes(path.join(".glitchcode", "thing.json"))).toBe(true)
  })
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".glitchcode", "thing.json"))).toBe(false)
  })
