...
// the live router + a real key in ~/.config/mimocode/glitchcode.json). Run with:
...
function realConfig() {
  const home = path.join(os.homedir(), ".config", "mimocode", "glitchcode.json")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .glitchcode/workflows.
            const wfDir = path.join(dir, ".glitchcode", "workflows")
