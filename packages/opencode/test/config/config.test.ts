...
const managedConfigDir = process.env.GLITCHCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "glitchcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "glitchcode.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "glitchcode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["GLITCHCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:GLITCHCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["GLITCHCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["GLITCHCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "glitchcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .glitchcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".glitchcode")
...
test("loads agents from .glitchcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".glitchcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .glitchcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".glitchcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .glitchcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".glitchcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".glitchcode", "command", "dup.md"),
        `---
...
test("does not try to install dependencies in read-only GLITCHCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.GLITCHCODE_CONFIG_DIR
  process.env.GLITCHCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.GLITCHCODE_CONFIG_DIR
    else process.env.GLITCHCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable GLITCHCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .glitchcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".glitchcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .glitchcode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "glitchcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .glitchcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "glitchcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".glitchcode")
      ...
      await Filesystem.write(
        path.join(local, "glitchcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets GLITCHCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .glitchcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .glitchcode directory config enables it
      const opencodeDir = path.join(dir, ".glitchcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "glitchcode.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "glitchcode.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.glitchcode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.glitchcode/plugin/demo.ts", "file:///project/.glitchcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.glitchcode/plugin/demo.ts"])
  })
...
describe("GLITCHCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .glitchcode/ directories when flag is set", async () => {
    const originalEnv = process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .glitchcode directory with a command
          const opencodeDir = path.join(dir, ".glitchcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .glitchcode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["GLITCHCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["GLITCHCODE_CONFIG_DIR"]
      process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["GLITCHCODE_CONFIG_DIR"]
      } else {
        process.env["GLITCHCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("GLITCHCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["GLITCHCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["GLITCHCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["GLITCHCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from GLITCHCODE_CONFIG_DIR, not project
...
describe("GLITCHCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in GLITCHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["GLITCHCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["GLITCHCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["GLITCHCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["GLITCHCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in GLITCHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["GLITCHCODE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.glitchcode.managed.test",
          PayloadType: "ai.glitchcode.managed",
