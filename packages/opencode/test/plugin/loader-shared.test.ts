...
const disableDefault = process.env.GLITCHCODE_DISABLE_DEFAULT_PLUGINS
process.env.GLITCHCODE_DISABLE_DEFAULT_PLUGINS = "1"
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.GLITCHCODE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.GLITCHCODE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.GLITCHCODE_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.GLITCHCODE_PURE
      } else {
        process.env.GLITCHCODE_PURE = pure
      }
    }
  })
