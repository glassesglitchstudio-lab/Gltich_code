...
describe("GLITCHCODE_HOME end-to-end", () => {
  test("sets all four paths under the profile root", async () => {
    await using tmp = await tmpdir()
    const result = await runWorker({ GLITCHCODE_HOME: tmp.path })
  ...
  test("auto-creates all four subdirs under a nonexistent root", async () => {
    ...
    const result = await runWorker({ GLITCHCODE_HOME: nested })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test("two profiles have fully isolated data (write in A, not visible in B, A survives restart)", async () => {
    ...
    const resultA = await runWorker({ GLITCHCODE_HOME: a.path })
    ...
    const resultB = await runWorker({ GLITCHCODE_HOME: b.path })
    ...
    const resultA2 = await runWorker({ GLITCHCODE_HOME: a.path })
  ...
  test("relative path causes startup failure with clear error", async () => {
    const result = await runWorker({ GLITCHCODE_HOME: "./relative-path" })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.stderr).toMatch(/GLITCHCODE_HOME must be an absolute path/)
  })
