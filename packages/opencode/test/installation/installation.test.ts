...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when glitchcode-cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "glitchcode-cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when glitchcode-cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "glitchcode-cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when glitchcode-cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "glitchcode-cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has glitchcode-cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("glitchcode-cli"))
    ...
    test("resolves version from GitHub releases redirect for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://github.com/glassesglitchstudio-lab/Gltich_code/releases/latest"))
            return "HTTP/2 302\r\nlocation: https://github.com/glassesglitchstudio-lab/Gltich_code/releases/tag/v0.1.1\r\n"
          return ""
        },
      )
    ...
    test("dies for unsupported channels (unknown)", async () => {
      const layer = testLayer(() => jsonResponse({}))
      const unsupported: Installation.Method[] = ["unknown"]
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("glitchcode-cli@2.0.0")
    })
