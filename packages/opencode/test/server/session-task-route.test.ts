...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.GLITCHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.GLITCHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.GLITCHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.GLITCHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
