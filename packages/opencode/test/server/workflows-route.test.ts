...
// process-global `:memory:` DB (test/preload sets GLITCHCODE_DB=:memory:, and the
...
// `x-glitchcode-directory` header makes InstanceMiddleware re-enter the SAME cached
      ...
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const response = yield* Effect.promise(async () =>
          Server.Default().app.request(`/workflows/wf_00000000000000000000000000/resume`, {
            method: "POST",
            headers: { "x-glitchcode-directory": dir },
          }),
        )
