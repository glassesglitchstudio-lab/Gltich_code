...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "glitchcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "glitchcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "glitchcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "glitchcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["glitchcode.json", "glitchcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "glitchcode.json"),
        path.join(tmp.path, "glitchcode.jsonc"),
        path.join(project, "glitchcode.json"),
        path.join(project, "glitchcode.jsonc"),
      ])
    })
  })
