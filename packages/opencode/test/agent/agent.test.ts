...
test("plan agent denies edits except .glitchcode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".glitchcode/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".glitchcode", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".glitchcode", "skill", "perm-skill")
