...
test("GitLab Duo: loads provider with API key from environment", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      set("GITLAB_TOKEN", "test-gitlab-token")
    },
    fn: async () => {
...
test("GitLab Duo: supports self-hosted instance configuration", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      set("GITLAB_INSTANCE_URL", "https://gitlab.company.internal")
    },
    fn: async () => {
      const providers = await list()
      expect(providers[ProviderID.gitlab]).toBeDefined()
...
describe("GitLab Duo: workflow model routing", () => {
  test("duo-workflow-* model routes through workflowChat", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "glitchcode.json"), JSON.stringify({ $schema: "https://opencode.ai/config.json" }))
      },
    })
    await Instance.provide({
      directory: tmp.path,
