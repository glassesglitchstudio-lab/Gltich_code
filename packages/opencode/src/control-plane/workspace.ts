...
import { Slug } from "@glitchcode/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    GLITCHCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    GLITCHCODE_WORKSPACE_ID: config.id,
    GLITCHCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.GLITCHCODE_EXPERIMENTAL_WORKSPACES) return
