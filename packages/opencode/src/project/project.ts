...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".glitchcode-project-id")
  const idFile = nodePath.join(mainGit, "glitchcode-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .glitchcode-project-id
  ...
  if (!existing.includes(".glitchcode-project-id")) {
    await nodeFs.appendFile(excludeFile, "\n.glitchcode-project-id\n")
  }
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.GLITCHCODE_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        if (Flag.GLITCHCODE_DISABLE_GIT) {
          return {
            id: ProjectID.global,
            worktree: directory,
            sandbox: directory,
        ...
        // turn, which has shipped as a bug for users opening glitchcode in $HOME.
      ...
      if (Flag.GLITCHCODE_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
