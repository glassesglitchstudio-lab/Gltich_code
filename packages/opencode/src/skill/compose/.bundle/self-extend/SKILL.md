...
You can extend your own capabilities by writing files to `.glitchcode/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.

## Creating Tools

Write to `.glitchcode/tools/<name>.ts`:

```ts
import { tool } from "@glitchcode/plugin"
...
Write to `.glitchcode/hooks/<name>.ts` — export a Hooks object:
...
```ts
// .glitchcode/tools/bash.ts — overrides built-in bash
import { tool } from "@glitchcode/plugin"
...
Write to `.glitchcode/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.glitchcode/tools/*.ts` | next turn |
| Hooks | `.glitchcode/hooks/*.ts` | next turn |
| Skills | `.glitchcode/skills/*/SKILL.md` | next turn |
| TUI | `.glitchcode/tui/*.tsx` | restart |
