...
const GLITCHCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.GLITCHCODE_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.GLITCHCODE_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.GLITCHCODE_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.GLITCHCODE_DISABLE_CODEX_SKILLS) return false
      if (dir === ".opencode" && Flag.GLITCHCODE_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, GLITCHCODE_SKILL_PATTERN)
  }
