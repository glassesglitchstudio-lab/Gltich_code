...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".glitchcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".glitchcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".glitchcode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.GLITCHCODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.GLITCHCODE_PURE && config.plugin_origins?.length) {
        ...
        // File-based TUI plugins from .glitchcode/tui/*.{ts,tsx}
        const configDirs = [
          path.join(Global.Path.config, "tui"),
          path.join(cwd, ".glitchcode", "tui"),
        ]
