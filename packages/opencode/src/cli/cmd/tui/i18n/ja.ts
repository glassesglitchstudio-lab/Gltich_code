...
export const dict = {
  ...
  "tui.home.agreement.prefix": "Glitch Code をご利用いただくことで、",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}glitchcode.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/glitchcode/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "Glitch Code は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.glitchcode/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.glitchcode/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "Glitch Code は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "Glitch Code はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.glitchcode/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.glitchcode/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて Glitch Code が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}glitch run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}glitch --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}glitch run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}glitch serve{/highlight} を実行するとヘッドレスで Glitch Code API にアクセスできます",
  "tui.tips.attach_server": "{highlight}glitch run --attach{/highlight} で起動中のサーバに接続します",
  ...
  "tui.tips.theme_files": "{highlight}.glitchcode/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}glitch debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.toast.update_available.success": "Glitch Code v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.dialog.login.glitch_free.desc": "ログイン不要の匿名無料チャネル",
  "tui.dialog.login.glitch_free.success": "Glitch Auto (free) の準備完了 — デフォルトモデルを glitch/glitch-auto に設定しました",
  "tui.dialog.login.glitch_free.unavailable": "Glitch Auto (free) プロバイダーが読み込まれていません",
  ...
  "cli.providers.glitch.recommended_hint": "推奨",
  "cli.providers.glitch_free.hint": "匿名無料チャネル / glitch-auto",
  "cli.providers.glitch_free.verifying": "Glitch Auto (free) チャネルを検証中...",
  "cli.providers.glitch_free.ready": "Glitch Auto (free) チャネル準備完了",
  "cli.providers.glitch_free.failed": "Glitch Auto (free) の自己診断に失敗",
  "cli.providers.glitch_free.default_set": "デフォルトモデルを glitch/glitch-auto に設定（1M コンテキスト、無料）",
  "cli.providers.glitch_free.usage_hint":
    "ログイン不要 — そのまま glitch を実行できます。有料/上位モデルを利用する場合は MiMo ブラウザログインを選択してください。",
  "cli.providers.glitch_login.decrypt_retry": "復号に失敗しました、再試行してください（残り {remaining} 回）",
  "cli.providers.glitch_login.decrypt_exhausted": "復号に失敗しました、最大再試行回数に達しました",
  ...
  "trust.capabilities": "Glitch Code はここでファイルの読み取り、編集、実行が可能になります。",
