...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.glitchcode`
  // folders between cwd and home, and GLITCHCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.GLITCHCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit GLITCHCODE_TUI_CONFIG override, if set.
  if (Flag.GLITCHCODE_TUI_CONFIG) {
    const configFile = Flag.GLITCHCODE_TUI_CONFIG
  ...
  // 4. `.glitchcode` directories (and GLITCHCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".glitchcode") || dir === Flag.GLITCHCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".glitchcode") && dir !== Flag.GLITCHCODE_CONFIG_DIR) continue
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old glitchcode.json shape) still get their settings applied.
