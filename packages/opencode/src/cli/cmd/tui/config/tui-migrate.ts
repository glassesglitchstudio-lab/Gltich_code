...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from glitchcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const glitchcode = await glitchcodeFiles(input)
  for (const file of glitchcode) {
...
async function glitchcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "glitchcode"),
    ...(await Filesystem.findUp(["glitchcode.json", "glitchcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "glitchcode"))
  }
  if (Flag.GLITCHCODE_CONFIG) files.push(Flag.GLITCHCODE_CONFIG)
