...
import type { GlobalEvent } from "@glitchcode/sdk/v2"
...
export const TuiThreadCommand = cmd({
  ...
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start glitchcode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [GLITCHCODE_PROCESS_ROLE]: "worker",
        [GLITCHCODE_RUN_ID]: ensureRunID(),
      })
