    ...
    <Dialog size="large" onClose={props.onClose}>
      ...
      >
        ...
        <box
          ...
          marginBottom={1}
        >
          <text color={CraftedTheme.success}>🩺 GLITCH AUTO-DOCTOR v2.1</text>
          <text color={CraftedTheme.textDim}>[Crafted Minimal TUI]</text>
        </box>

        {/* Diagnostics Info */}
        <box flexDirection="column" paddingLeft={1} paddingRight={1} marginBottom={1}>
          <box flexDirection="row">
            <text color={CraftedTheme.danger} bold>[HATA] </text>
            <text color={CraftedTheme.text}>{props.issue.substring(0, 65)}</text>
          </box>
          <box flexDirection="row" marginTop={0}>
            <text color={CraftedTheme.warning} bold>[KÖK NEDEN] </text>
            <text color={CraftedTheme.textMuted}>{props.rootCause.substring(0, 60)}</text>
          </box>
          <box flexDirection="row" marginTop={0}>
            <text color={CraftedTheme.info} bold>[GÜVEN] </text>
            <text color={CraftedTheme.success}>{confidenceStars()} </text>
            <text color={CraftedTheme.textDim}>
        ...
        <Show when={activeTab() === "menu"}>
          <box flexDirection="column" backgroundColor={CraftedTheme.bgSubtle} padding={1} marginBottom={1}>
            <text color={CraftedTheme.textMuted} bold>
              ⚡ EYLEM MENÜSÜ:
            </text>
            <For each={menuItems}>
              {(item, i) => {
                ...
                return (
                  <box
                    ...
                    marginTop={0}
                  >
                    <text color={isSelected() ? CraftedTheme.success : CraftedTheme.textDim} bold>
                      [{item.key}]{" "}
                    </text>
                    <text color={isSelected() ? CraftedTheme.text : CraftedTheme.textMuted}>{item.label}</text>
                  </box>
                )
              }}
            </For>
        ...
        <Show when={activeTab() === "diff"}>
          <box flexDirection="column" backgroundColor={CraftedTheme.diff.gutterBg} padding={1} marginBottom={1}>
            <text color={CraftedTheme.info} bold>
              📄 DIFF İNCELEME ({props.filePath || "Değiştirilen Dosya"}):
            </text>
            <box flexDirection="column" marginTop={1}>
              <text color={CraftedTheme.diff.removedFg}>- {props.diffPreview || "Eski hatalı kod bloğu"}</text>
              <text color={CraftedTheme.diff.addedFg}>+ {(props.diffPreview || "").replace("-", "+") || "Düzeltilmiş optimize kod bloğu"}</text>
            </box>
            <box marginTop={1}>
              <text color={CraftedTheme.textDim}>[Esc/q ile menüye dön]</text>
            </box>
          </box>
        </Show>
        ...
        <Show when={activeTab() === "explanation"}>
          <box flexDirection="column" backgroundColor={CraftedTheme.bgSubtle} padding={1} marginBottom={1}>
            <text color={CraftedTheme.warning} bold>
              💬 DOKTOR AÇIKLAMASI:
            </text>
            <text color={CraftedTheme.text} marginTop={1}>
              {props.explanation ||
        ...
        <Show when={statusMessage()}>
          <box backgroundColor={CraftedTheme.bgElevated} paddingLeft={1} marginBottom={1}>
            <text color={CraftedTheme.warning}>{statusMessage()}</text>
          </box>
        </Show>

        <box flexDirection="row" justifyContent="space-between" paddingLeft={1} paddingRight={1}>
          <text color={CraftedTheme.textDim}>[↑/↓ Gezin | 1-5 Hızlı Tuş | Enter Onayla]</text>
          <text color={CraftedTheme.textDim}>[Esc: Çıkış]</text>
        </box>
      </box>
    </Dialog>
  )
