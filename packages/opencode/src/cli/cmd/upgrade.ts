...
export const UpgradeCommand = {
  ...
  describe: "upgrade glitchcode to the latest or a specific version",
  builder: (yargs: Argv) => {
    return yargs
      .positional("target", {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (method === "unknown") {
      prompts.log.error(`glitchcode is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (InstallationVersion === target) {
      prompts.log.warn(`glitchcode upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }
