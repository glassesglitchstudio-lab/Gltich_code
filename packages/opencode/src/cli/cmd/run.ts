...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@glitchcode/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run glitchcode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running glitchcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to GLITCHCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.GLITCHCODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.GLITCHCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.GLITCHCODE_SERVER_USERNAME ?? "glitchcode"
