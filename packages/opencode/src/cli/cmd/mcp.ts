    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: glitch mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in glitchcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .glitchcode/ subdirectory too)
  const candidates = [path.join(baseDir, "glitchcode.json"), path.join(baseDir, "glitchcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".glitchcode", "glitchcode.json"), path.join(baseDir, ".glitchcode", "glitchcode.jsonc"))
  }
  ...
  // Default to glitchcode.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., glitch x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "glitchcode-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
