...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in glitchcode, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's glitchcode-native continuation messages in the same session.
          ...
          // Preserve glitchcode-owned metadata on re-sync: keep any user rename
