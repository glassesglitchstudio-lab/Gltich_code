...
const FILES = [
  "AGENTS.md",
  ...(Flag.GLITCHCODE_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.GLITCHCODE_CONFIG_DIR) {
    files.push(path.join(Flag.GLITCHCODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.GLITCHCODE_DISABLE_CLAUDE_CODE_PROMPT) {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.GLITCHCODE_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no GLITCHCODE_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.GLITCHCODE_CONFIG_DIR, Flag.GLITCHCODE_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
