...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-glitchcode-directory")
  out.delete("x-glitchcode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-glitchcode-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-glitchcode-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
