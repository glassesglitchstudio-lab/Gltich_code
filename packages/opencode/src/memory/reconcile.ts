...
export async function indexFromDisk(
  ...
  // For CC files, derive type from frontmatter; glitch files keep loc.type from path.
  const finalType =
...
export async function reconcileMemory(
  ...
  let indexedCount = 0
  for (const p of glitchFiles) {
    const loc = parsePath(p)
    if (!loc) {
      log.warn("path outside memory layout, skipping", { path: p })
      continue
    }
    const result = await indexFromDisk(p, loc, "glitch", indexed.get(p))
